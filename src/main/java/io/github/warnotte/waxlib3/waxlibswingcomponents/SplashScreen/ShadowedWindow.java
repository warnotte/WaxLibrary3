...
package io.github.warnotte.waxlib3.waxlibswingcomponents.SplashScreen;
...
public class ShadowedWindow extends JWindow implements MouseListener{
    ...
    private void createShadowPicture(BufferedImage image, boolean createShadow) {
    	this.Create_Shadow=createShadow;;
        ...
        if (createShadow==true)
        	...
        	splash_orig = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        ...
        try {
            ...
            if (createShadow==true)
            	capture = robot.createScreenCapture(new Rectangle(windowRect.x, windowRect.y, windowRect.width + extra, windowRect.height + extra));
            else
            	capture = robot.createScreenCapture(new Rectangle(windowRect.x, windowRect.y, windowRect.width , windowRect.height ));
        ...
        if (createShadow==true)
        	...
        	splash = new BufferedImage(width , height , BufferedImage.TYPE_INT_RGB);
